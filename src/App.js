    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f5f5f3'}}>
      <div style={{background:'#fff',border:'0.5px solid #e5e5e3',borderRadius:12,padding:'2rem',width:340}}>
        ...
        <input type="password" value={pw} onChange={e=>{setPw(e.target.value);setErr('');}} placeholder="Enter password"
          style={{width:'100%',padding:'8px 10px',borderRadius:8,border:'0.5px solid #d5d5d3',fontSize:13,marginBottom:err?6:16,display:'block'}} autoFocus />
        {err && <p style={{color:'#A32D2D',fontSize:13,margin:'0 0 12px'}}>{err}</p>}
        <button onClick={()=>pw===ADMIN_SECRET?onLogin():setErr('Incorrect password.')}
...
const HEADERS = h => ({'Content-Type':'application/json','x-admin-secret':h});
...
function Badge({label,bg,c}){return <span style={{fontSize:11,padding:'2px 8px',borderRadius:20,fontWeight:600,background:bg,color:c}}>{label}</span>;}
...
function Dashboard({ onLogout }) {
  ...
  const load=useCallback(async()=>{
    try{const r=await fetch(`${API}/api/tickets`,{headers:HEADERS(ADMIN_SECRET)});setTickets(await r.json());}
  ...
  async function save(){
    ...
    setSaving(true);
    try{
      if(modal==='new'){await fetch(`${API}/api/tickets/submit`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...form,submittedBy:'Benjamin (Admin)'}),});}
      else{await fetch(`${API}/api/tickets/${modal.id}`,{method:'PATCH',headers:HEADERS(ADMIN_SECRET),body:JSON.stringify(form)});}
  ...
  async function del(){
    ...
    await fetch(`${API}/api/tickets/${modal.id}`,{method:'DELETE',headers:HEADERS(ADMIN_SECRET)});
    await load();setModal(null);
  }

  const inp={display:'block',width:'100%',fontSize:13,padding:'8px 10px',borderRadius:8,border:'0.5px solid #d5d5d3',marginBottom:12,fontFamily:'inherit'};
  const sel={...inp};

  return(
    <div style={{maxWidth:860,margin:'0 auto',padding:'1.25rem 1rem',fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.25rem'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button onClick={()=>{setForm({title:'',desc:'',project:'',priority:'Medium',status:'To Do',due:''});setModal('new');}}
            style={{fontSize:13,padding:'6px 14px',borderRadius:8,border:'none',background:'#CC1818',color:'#fff',fontWeight:600,cursor:'pointer'}}>
            + New ticket
          </button>
          <button onClick={onLogout} style={{fontSize:12,padding:'6px 12px',borderRadius:8,border:'0.5px solid #d5d5d3',background:'#fff',cursor:'pointer'}}>Sign out</button>
        </div>
      </div>

      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:'1rem'}}>
        {[['Total',tickets.length,'#111'],['To do',tickets.filter(t=>t.status==='To Do').length,'#0C447C'],['In progress',tickets.filter(t=>t.status==='In Progress').length,'#633806'],['Done',tickets.filter(t=>t.status==='Done').length,'#27500A']].map(([l,v,c])=>(
          <div key={l} style={{background:'#f5f5f3',borderRadius:10,padding:'0.75rem 1rem'}}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:'1rem',flexWrap:'wrap'}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" style={{...inp,flex:1,minWidth:140,marginBottom:0}}/>
        <select value={fP} onChange={e=>setFP(e.target.value)} style={{...sel,marginBottom:0}}>
          <option value="">All projects</option>{projs.map(p=><option key={p}>{p}</option>)}
        </select>
        <select value={fPri} onChange={e=>setFPri(e.target.value)} style={{...sel,marginBottom:0}}>
          <option value="">All priorities</option>{['High','Medium','Low'].map(p=><option key={p}>{p}</option>)}
        </select>
        <select value={fSt} onChange={e=>setFSt(e.target.value)} style={{...sel,marginBottom:0}}>
          <option value="">All statuses</option>{['To Do','In Progress','Done'].map(p=><option key={p}>{p}</option>)}
        </select>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {filtered.map(t=>{
          const over=t.due&&t.status!=='Done'&&new Date(t.due)<new Date(new Date().toDateString());
          return(
            <div key={t.id} onClick={()=>{setForm({...t});setModal(t);}}
              ...
              <div style={{display:'flex',gap:6,flexWrap:'wrap',paddingLeft:64,alignItems:'center'}}>
                <Badge label={t.status} {...(ST[t.status]||ST['To Do'])}/>
                <Badge label={t.priority} {...(PRI[t.priority]||PRI.Medium)}/>
                {t.project&&<Badge label={t.project} bg="#EEEDFE" c="#3C3489"/>}
                {t.submittedBy&&<span style={{fontSize:11,color:'#999'}}>by {t.submittedBy}</span>}
                {t.due&&<span style={{fontSize:11,padding:'2px 8px',borderRadius:20,background:over?'#FCEBEB':'#f0f0ee',color:over?'#791F1F':'#666'}}>📅 {t.due}{over?' · overdue':''}</span>}
              </div>
      ...
      {modal&&(
        <div onClick={e=>{if(e.target===e.currentTarget)setModal(null);}}
          ...
          <div style={{background:'#fff',borderRadius:12,border:'0.5px solid #e5e5e3',padding:'1.25rem',width:'min(540px,94vw)',maxHeight:'88vh',overflowY:'auto'}}>
            ...
            <textarea value={form.desc||''} onChange={e=>setForm(f=>({...f,desc:e.target.value}))} rows={3} style={{...inp,resize:'vertical'}}/>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              <div>
                <label style={{fontSize:12,color:'#666',display:'block',marginBottom:4}}>Project</label>
                <input value={form.project||''} onChange={e=>setForm(f=>({...f,project:e.target.value}))} style={inp}/>
              </div>
              <div>
                <label style={{fontSize:12,color:'#666',display:'block',marginBottom:4}}>Priority</label>
                <select value={form.priority||'Medium'} onChange={e=>setForm(f=>({...f,priority:e.target.value}))} style={sel}>
                  {['High','Medium','Low'].map(p=><option key={p}>{p}</option>)}
                </select>
              </div>
              <div>
                <label style={{fontSize:12,color:'#666',display:'block',marginBottom:4}}>Status</label>
                <select value={form.status||'To Do'} onChange={e=>setForm(f=>({...f,status:e.target.value}))} style={sel}>
              ...
              <div>
                ...
                <input type="date" value={form.due||''} onChange={e=>setForm(f=>({...f,due:e.target.value}))} style={inp}/>
              </div>
            </div>
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:'1rem',paddingTop:'0.75rem',borderTop:'0.5px solid #e5e5e3'}}>
              {modal!=='new'&&<button onClick={del} style={{fontSize:13,padding:'7px 14px',borderRadius:8,border:'0.5px solid #F09595',background:'#fff',color:'#A32D2D',cursor:'pointer'}}>Delete</button>}
              ...
              <button onClick={save} disabled={saving} style={{fontSize:13,padding:'7px 14px',borderRadius:8,border:'none',background:'#CC1818',color:'#fff',fontWeight:600,cursor:'pointer'}}>{saving?'Saving…':'Save'}</button>
            </div>
          </div>
        </div>
      )}
...
function SubmitForm() {
  ...
  async function submit(){
    ...
    try{
      const r=await fetch(`${API}/api/tickets/submit`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});
      if(!r.ok)throw new Error();
      const d=await r.json();setTicketId(d.ticket?.id||'');setDone(true);
    }catch{setError('Could not submit. Please try again.');}
    finally{setSubmitting(false);}
  }

  if(done)return(
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f5f5f3',padding:'1rem',fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif'}}>
  ...
  return(
    <div style={{minHeight:'100vh',background:'#f5f5f3',padding:'1.5rem 1rem',fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif'}}>
      <div style={{maxWidth:520,margin:'0 auto'}}>
        ...
        <div style={{background:'#fff',border:'0.5px solid #e5e5e3',borderRadius:12,padding:'1.25rem'}}>
          ...
          <textarea value={form.desc} onChange={e=>set('desc',e.target.value)} placeholder="Any additional context…" rows={4} style={{...inp,resize:'vertical'}}/>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            <div>
          ...
          <input type="date" value={form.due} onChange={e=>set('due',e.target.value)} style={{...inp,marginBottom:'1.25rem'}}/>
